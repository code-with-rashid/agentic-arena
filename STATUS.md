...
| `smolagents` `CodeAgent` | **shipped** as `frameworks/smolagents_code` (contrast entry, `tool_use` only) — 15/15 in mock mode. Needed a new `arena.llm.mockserver` accommodation to render a scripted turn as a Python `<code>` blob. | widen its `arenas` beyond `tool_use` if `rag` / `structured_output` prove clean |
