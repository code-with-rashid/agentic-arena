...
| `frameworks/crewai/adapter.py` | written, **not mock-verified** — CrewAI's transitive tree (chromadb/onnxruntime) has no Python 3.14 wheels. On 3.12 (crewai 0.203.2) it installs, builds and answers correctly but scores **0/15**: it drives a text ReAct loop, the wrapper tool-call sink stays empty, and every `tool_used` check fails. Also emits an interactive "view execution traces? [y/N]" prompt that `CREWAI_TRACING_ENABLED=false` no longer suppresses. Kept out of the required matrix. | give the mock a CrewAI-shaped ReAct accommodation (or capture tool calls from the crew result), silence the trace prompt on 0.203.2, pin the version, add its CI cell, refresh results |
