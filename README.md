...
- **Honest scorecards.** Pass rate vs. a graded eval set, latency, token usage,
  estimated cost, retries, and crashes, rendered as both a table and hand-written
  SVG bar charts (no plotting dependency) — plus a feature matrix for the things
  that don't reduce to a number.
- **Self-auditing.** `results/REFRESHED.md` rolls every published scorecard into
  one "last refreshed on \<date\> with \<versions\>" table, regenerated by CI so
  published numbers can't quietly go stale.

> **Status:** Phases 0–3 done, Phase 4 (polish) in progress. All seven arenas run;
> seven framework adapters (plus a `CodeAgent` contrast entry) and five `*_multi`
> pipeline entries are green against the mock; the harness core is still
> stdlib-only. What is left is a **live scorecard** (needs an API key — every
> number so far is mock mode, so nothing yet says which framework gives *better
> answers*), `crewai` full verification, `claude_agent_sdk` (a stub, protocol
> mismatch), and publishing the [docs site](#documentation) to GitHub Pages. See
> [ROADMAP.md](ROADMAP.md) and [docs/next-phases.md](docs/next-phases.md);
